...
void back_decryption_k() {
  for (int i = 0; i < 32; i++) {
    back_aes_key[i] = aes_key[i];
  }
}

void rest_decryption_k() {
  for (int i = 0; i < 32; i++) {
...
void encrypt_with_aes(int to_be_encr[]) {
  ...
  int i = 0;
  aes_context ctx;
  aes_set_key(&ctx, aes_key, aes_mode[m]);
...
void decrypt_string_with_aes_in_cbc(String ct) { // Function for decryption. Takes ciphertext as an input.
  back_key();
  clear_variables();
